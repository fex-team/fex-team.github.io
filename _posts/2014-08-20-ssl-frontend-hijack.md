...
事实上，超链接无论是谁产生的、何时添加进来的，只要不去点击，都是不起作用的。所以，我们只需关心何时去点击就可以 —— 如果我们的程序，能在点击产生的第一时间里控制住现场，那么之后的流程就可由我们决定了。

听起来似乎很玄乎，不过要在前端中实现，只是小菜一碟的事。点击，不过个事件而已。既然是事件，我们用最基础的事件捕获机制，即可将其轻松拿下：
...
这在过去是个很棘手的问题，然而 HTML5 时代给我们带来了新希望 —— MutationEvent。用它即可实时监控页面元素，之前也写过[一些试验](http://fex.baidu.com/blog/2014/06/xss-frontend-firewall-2/)。
...
通过派遣的前端间谍，我们轻易解决了过去各种棘手的问题。但挑战并未就此结束，我们仍面临着众多难题。
...
不过，如今的 Web 里增加一个新的安全标准：[HTTP Strict Transport Security](https://www.owasp.org/index.php/HTTP_Strict_Transport_Security)。如果服务器返回了这个头部，之后一定时间内访问该站点，始终通过 HTTPS 的方式。

所以我们的中间人一旦发现这个头，就得果断将其删除。

当然，直接敲网址的并不常见。大多都是搜索 zhifubao，然后直接从第一个结果里进来了。
...
![](/img/ssl-frontend-hijack/js_login.png)
...
[https://github.com/EtherDream/https_hijack_demo](https://github.com/EtherDream/https_hijack_demo)
...
当然之前也说了，并非所有的页面都能劫持成功。如今越来越多的网站都已重视，因此前端的安全性检测也随之而生。仅仅通过一个工具，实现大规模通用化的劫持，未来会更加困难。
